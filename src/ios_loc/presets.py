"""Paces and named route presets, loaded from TOML."""
...
class Pace:
    """How fast a mode of travel moves, and how it rests.

    Speed only — a pace says nothing about how a route is planned. Which roads
    or paths the route follows is the Valhalla costing model, chosen
    independently (`--costing`, the GUI's "Routing mode", a preset's own
    `costing`). Keeping the two apart is deliberate: picking `bike` for its
    speed must not silently re-route a pedestrian path onto cycleways.
    """
    ...
    def __post_init__(self) -> None:
        if self.speed <= 0:
            raise ValueError(f"pace {self.name!r}: speed must be positive")
        if self.speed > MAX_SPEED_MPS:
            raise ValueError(
                f"pace {self.name!r}: speed {self.speed} m/s "
                f"({self.speed * 3.6:.1f} km/h) exceeds the {MAX_SPEED_MPS} m/s "
                f"(20 km/h) ceiling; movement above it is not credited"
            )
        if self.jitter < 0:
            raise ValueError(f"pace {self.name!r}: jitter must not be negative")
        if self.pause_per_min < 0:
            raise ValueError(f"pace {self.name!r}: pause_per_min must not be negative")
        if self.pause_min_s < 0:
            raise ValueError(f"pace {self.name!r}: pause_min_s must not be negative")
        if self.pause_min_s > self.pause_max_s:
            raise ValueError(
                f"pace {self.name!r}: pause_min_s ({self.pause_min_s}) exceeds "
                f"pause_max_s ({self.pause_max_s})"
            )


# What a route is planned with when nothing says otherwise. A `Pace` no longer
# carries a costing, so this is the one fallback for an ad-hoc route.
DEFAULT_COSTING = "pedestrian"


@dataclass(frozen=True)
class Preset:
    """A named route: its geometry, the costing it was planned with, and the
    pace to walk it at.

    `costing` is stored per route rather than derived from the pace, because it
    describes the saved *geometry* — re-planning a saved route under a different
    costing yields a different polyline than the one the user drew and saved.
    """
...
def load_config(
...
) -> tuple[dict[str, Pace], dict[str, Preset]]:
    """Load paces and presets, layering the TOML file over the built-in defaults."""
    path = pathlib.Path(path) if path else DEFAULT_CONFIG_PATH
    paces = dict(DEFAULT_PACES)
    presets: dict[str, Preset] = {}

    if not path.exists():
        return paces, presets

    data = tomllib.loads(path.read_text())

    for name, raw in data.get("paces", {}).items():
        # A brand-new pace name inherits `walk`'s defaults for any field the
        # TOML omits; an existing name layers over its own current values.
        base = paces.get(name, DEFAULT_PACES["walk"])
        try:
            paces[name] = replace(
                base,
                name=name,
            ...
            raise ConfigError(f"pace {name!r} in {path}: {exc}") from exc
    ...
    for name, raw in data.get("presets", {}).items():
        ...
        pace_name = str(raw.get("pace", "walk"))
        if pace_name not in paces:
            raise ConfigError(
                f"preset {name!r} in {path}: unknown pace {pace_name!r}; "
                f"available: {', '.join(sorted(paces))}"
            )
        presets[name] = Preset(
            name=name,
            ...
            costing=str(raw.get("costing", DEFAULT_COSTING)),
        )
...
def resolve_walk(
    *,
    ...
    if pace_name not in paces:
        raise ConfigError(f"unknown pace {pace_name!r}")
    resolved_pace = paces[pace_name]

    if speed is not None:
        # Includes the 20 km/h ceiling — never bypass this validation.
        resolved_pace = replace(resolved_pace, speed=speed)

    return ResolvedWalk(
        waypoints=resolved_waypoints,
        costing=resolved_costing,
        pace=resolved_pace,
        loop=resolved_loop,
...
def _strip_tables(text: str, kind: str) -> str:
    ...
    regenerates only its own kind, so a hand-written [paces.*] section and its
...
def save_preset(path: pathlib.Path | None, preset: Preset) -> None:
    ...
    preset tables are not. `preset.pace` is validated against the built-in
    paces plus any `[paces.*]` the file already defines. The file is read
    first to load existing presets; if it is already invalid — for example an
    existing preset references a pace that no longer exists — `save_preset`
    raises `ConfigError` without writing, and the file must be repaired by hand.
    If the new preset's pace is unknown, the file is left untouched instead
    ...
    # this is also where we learn the paces a not-yet-written file would
    # accept. Validating here -- before any read-modify-write of the target
    # file -- means a preset naming an unknown pace never touches disk, and
    # never jams `load_config` for every save that follows.
    paces, existing = load_config(path)
    if preset.pace not in paces:
        raise ConfigError(
            f"preset {preset.name!r}: unknown pace {preset.pace!r}; "
            f"available: {', '.join(sorted(paces))}"
        )
...
def save_place(path: pathlib.Path | None, place: Place) -> None:
    ...
    including line endings and the [presets.*] and [paces.*] sections. An
